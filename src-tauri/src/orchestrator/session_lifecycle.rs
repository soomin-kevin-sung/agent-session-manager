use crate::db::{self, DbPool, sessions::CreateSession};
use crate::AppResult;
...
impl SessionLifecycle {
    ...
    pub async fn create_work_session(
        ...
        let channel = db::channels::create(pool, &db::channels::CreateChannel {
        ...
        let session = db::sessions::create(pool, &CreateSession {
            workspace_id: workspace_id.into(),
...
mod tests {
    ...
    async fn test_create_work_session() {
        ...
        let agent = db::agents::create(&pool, &db::agents::CreateAgent {
            name: "Dev".into(), runtime_type: "claude_cli".into(), provider: "anthropic".into(),
        ...
        let members = db::sessions::list_members(&pool, &session.id).await.unwrap();
        assert_eq!(members.len(), 1);

        let ch_members = db::channels::list_members(&pool, &session.channel_id).await.unwrap();
